...
function App() {
  ...
  return (
    <main className="enterprise-shell">
      ...
      <div className={adminAuditOpen ? "app-shell audit-mode" : inspectorOpen ? "app-shell" : "app-shell inspector-collapsed"}>
      <aside className="sidebar">
        ...
        {isAdmin ? <AdminConversationAuditPanel active={adminAuditOpen} onOpen={() => setAdminAuditOpen(true)} /> : null}
      ...
      <section className={adminAuditOpen ? "workspace workspace-hidden" : "workspace"}>
      ...
      {adminAuditOpen ? <AdminConversationAuditWorkspace onClose={() => setAdminAuditOpen(false)} /> : null}

      {adminAuditOpen ? null : inspectorOpen ? <aside className="sources-panel">
...
function AdminConversationAuditPanel({ active, onOpen }: { active: boolean; onOpen: () => void }) {
  return (
    <section className="control-section admin-audit-panel">
      <div className="section-heading"><FileSearch size={16} /><span>会话审计</span></div>
      <p className="audit-notice">在中间工作区查看用户与完整会话内容。</p>
      <button className={active ? "primary-button" : "secondary-button"} type="button" onClick={onOpen} aria-current={active ? "page" : undefined}>
        {active ? "正在查看用户会话" : "查看用户会话"}
      </button>
    </section>
  );
}

function AdminConversationAuditWorkspace({ onClose }: { onClose: () => void }) {
  ...
  return (
    <section className="audit-workspace" aria-label="用户会话审计工作区">
      <header className="audit-workspace-header">
        <div>
          <div className="section-heading"><FileSearch size={16} /><span>会话审计</span></div>
          <p>只读查看用户历史会话，审计访问会被记录。</p>
        </div>
        <div className="audit-header-actions">
          <button className="secondary-button" type="button" onClick={() => void loadUsers()} disabled={busy}>
            {busy ? <Loader2 className="spin" size={15} /> : <RefreshCcw size={15} />} 刷新
          </button>
          <button className="secondary-button" type="button" onClick={onClose}><ArrowLeft size={15} />返回问答</button>
        </div>
      </header>

      <div className="audit-workspace-body">
        <div className="audit-workspace-error-slot">
          {error ? <p className="audit-workspace-error login-error">{error}</p> : null}
        </div>
        <div className="audit-browser">
        <section className="audit-browser-column audit-users-column">
          <div className="audit-column-heading">
            <span>用户</span>
            <small>{users.length}</small>
          </div>
          <div className="audit-workspace-list" aria-label="用户列表">
            {users.map((user) => (
              <button
        ...
        <section className="audit-browser-column audit-conversations-column">
          <div className="audit-column-heading">
            <span>{selectedUser ? `${selectedUser.username} 的会话` : "会话"}</span>
            <small>{selectedUser ? conversations.length : "—"}</small>
          </div>
          {selectedUser ? (
              <div className="audit-workspace-list" aria-label="会话列表">
                {conversations.map((conversation) => (
                  <button
          ...
          ) : <AuditWorkspacePrompt text="先从左侧选择一个用户" />}
        </section>

        <section className="audit-browser-column audit-message-column">
          <div className="audit-column-heading audit-message-heading">
            <span>{selectedConversation?.title || "会话内容"}</span>
            <small>{selectedConversation ? `${messages.length} 条消息` : "只读"}</small>
          </div>
          {selectedConversation ? (
              <div className="audit-workspace-messages" aria-live="polite">
                {messages.map((message, index) => (
                  <article className={`audit-workspace-message ${message.role}`} key={`${message.role}-${index}`}>
                    <div className="audit-message-meta">
                      <span>{message.role === "user" ? selectedUser?.username || "用户" : "QueryNest"}</span>
                      {message.created_at ? <time>{formatAuditTime(message.created_at)}</time> : null}
                    </div>
                    <div className="audit-message-content"><ReactMarkdown>{message.content}</ReactMarkdown></div>
                  </article>
                ))}
                {messages.length === 0 && !busy ? <p className="muted">该会话暂无消息。</p> : null}
              </div>
          ) : <AuditWorkspacePrompt text="选择会话后将在这里显示完整内容" />}
        </section>
        </div>
      </div>
    </section>
...
function AuditWorkspacePrompt({ text }: { text: string }) {
  return <div className="audit-workspace-prompt"><MessageSquare size={24} /><p>{text}</p></div>;
}
