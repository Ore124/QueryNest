...
class PersonalMemoryService:
    ...
    def _extract(self, user_message: str) -> tuple[list[PersonalMemoryCandidate] | None, str | None]:
        ...
        fallback_candidates = _extract_explicit_personal_memories(
            user_message,
            max_items=self.extraction_max_items,
        )
...
_PERSONAL_MEMORY_EXTRACTION_PROMPT = (
    '你是个人长期记忆提取器。只提取用户明确使用“我/我的/本人”陈述、且适合跨会话保留的信息。'
    '一条消息可以同时包含个人陈述和问题；必须提取其中的个人陈述，不要因为后面附带问题而返回空数组。'
    '以下内容只要被用户明确陈述就必须提取：职业或背景用 profile；正在持续学习或重点关注的主题用 fact；'
    '希望以后采用的回答结构、详略或表达方式用 preference。仅仅说“我想问/我想知道某个问题”不属于长期记忆。'
    '不得推断，不得保存他人信息、密码或令牌、联系方式、证件、健康、财务信息。'
    '只返回原始 JSON，不要 Markdown 代码块或解释，格式必须严格为 {"memories":[...]}。每项必须包含 '
    'memory_type、key、value、confidence、evidence；ttl_days 可选。memory_type 只能是 preference、profile、fact；'
    'evidence 必须逐字复制用户原文中包含“我/我的/本人”的片段；confidence 为 0 到 1；ttl_days 为 1 到 365。'
    '示例输入：我是一名前端开发者，目前重点学习 XSS；我偏好先给防御结论，再解释攻击原理。请说明存储型 XSS。'
    '示例输出：{"memories":['
    '{"memory_type":"profile","key":"职业背景","value":"前端开发者","confidence":0.98,"evidence":"我是一名前端开发者"},'
    '{"memory_type":"fact","key":"当前学习重点","value":"XSS","confidence":0.95,'
    '"evidence":"我是一名前端开发者，目前重点学习 XSS"},'
    '{"memory_type":"preference","key":"回答方式","value":"先给防御结论，再解释攻击原理","confidence":0.98,'
    '"evidence":"我偏好先给防御结论，再解释攻击原理"}]}。'
    '示例输入：我想知道存储型 XSS 和反射型 XSS 有什么区别？ 示例输出：{"memories":[]}。'
)
...
def parse_personal_memory_extraction(
    ...
    if normalized_user is None:
        return None
    try:
        raw = json.loads(_unwrap_json_code_fence(content))
...
def _unwrap_json_code_fence(content: str) -> str:
    """Accept a single JSON code fence while keeping all other parser checks strict."""
    fenced = re.fullmatch(
        r"\s*```(?:json)?[ \t]*\r?\n(.*?)\r?\n?```\s*",
        content,
...
def _extract_explicit_personal_memories(
    user_message: str, *, max_items: int = MAX_PERSONAL_MEMORIES_PER_TURN
) -> list[PersonalMemoryCandidate]:
    """Conservative fallback for explicit Chinese self-statements the model may omit."""
    normalized = _normalize_personal_text(user_message, MAX_PERSONAL_MEMORY_VALUE_CHARS)
    if normalized is None or not 1 <= max_items <= MAX_PERSONAL_MEMORIES_PER_TURN:
        return []

    candidates: list[PersonalMemoryCandidate] = []
    seen: set[tuple[str, str]] = set()

    def add_match(memory_type: str, key: str, match: re.Match[str], *, confidence: float) -> None:
        if len(candidates) >= max_items:
            return
        evidence = _normalize_personal_text(match.group(1), MAX_PERSONAL_MEMORY_EVIDENCE_CHARS)
        value = _normalize_personal_text(match.group(2), MAX_PERSONAL_MEMORY_VALUE_CHARS)
        identity = (memory_type, key.casefold())
        if (
            evidence is None
            or value is None
            or evidence not in normalized
            or not _is_explicit_first_person(evidence)
            or _contains_sensitive_data(f"{key} {value} {evidence}")
            or identity in seen
        ):
            return
        seen.add(identity)
        candidates.append(PersonalMemoryCandidate(memory_type, key, value, confidence, None, evidence))

    role = re.search(
        r"(我(?:目前)?(?:是一名|是一个|是|的职业是)\s*([^，。；！？!?]{2,40}))",
        normalized,
    )
    if role:
        add_match("profile", "职业背景", role, confidence=0.94)

    learning = re.search(
        r"(我(?:目前)?(?:正在)?(?:系统)?学习\s*([^，。；！？!?]{1,80}))",
        normalized,
    )
    if learning:
        add_match("fact", "持续学习主题", learning, confidence=0.93)

    focus = re.search(
        r"(我[^。；！？!?]{0,100}?(?:目前)?重点(?:学习|关注)\s*([^，。；！？!?]{1,80}))",
        normalized,
    )
    if focus:
        add_match("fact", "重点关注主题", focus, confidence=0.93)

    preference = re.search(r"(我偏好\s*([^。；！？!?]{2,160}))", normalized)
    if preference:
        add_match("preference", "回答方式", preference, confidence=0.95)

    desired_answer = re.search(
        r"(我希望\s*((?:以后|今后|回答时|答案|安全问题的回答)[^。；！？!?]{1,160}))",
        normalized,
    )
    if desired_answer:
        add_match("preference", "回答方式", desired_answer, confidence=0.92)
